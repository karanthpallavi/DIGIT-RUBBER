...
in the catalog file to redirect the URL `http://purl.obolibrary.org/obo/digitrubber/imports/go_import.owl`
...
Now add a new goal in your custom Makefile (`src/ontology/digitrubber.Makefile`, _not_ `src/ontology/Makefile`).
...
1. Providing a `components/mycomp.owl:` make target in `src/ontology/digitrubber.Makefile`
...
1. Providing an additional attribute for the component in `src/ontology/digitrubber-odk.yaml`, `source`,
